...
test('skill precedence: project .claude > DSH bundled (600) > ~/.claude', async () => {
  ...
  assert.deepEqual([...byName.keys()].sort(), ['bundled-only', 'shared', 'slash-one', 'user-only'], 'catalog exposes the four skills');
  ...
  assert.deepEqual(namesNoCwd, ['bundled-only', 'shared', 'slash-one', 'user-only'], 'without cwd only user ~/.claude + bundled are visible');
