...
test('skill precedence: project .claude > DSH bundled (600) > ~/.claude', async () => {
  ...
  assert.deepEqual([...byName.keys()].sort(), ['bundled-only', 'cc-plugin', 'reload-cc-plugins', 'reload-skills', 'shared', 'slash-one', 'user-only'], 'catalog exposes the four discovery skills + built-in plugin manager');
  ...
  assert.deepEqual(namesNoCwd, ['bundled-only', 'cc-plugin', 'reload-cc-plugins', 'reload-skills', 'shared', 'slash-one', 'user-only'], 'without cwd only user ~/.claude + bundled (+ plugin manager & reload skills) are visible');
